...
class Tfcampaign_Installer extends Engine_Package_Installer_Module {
	...
	protected function _addManagePage() {
        ...
        $page_id = $db->select()
            ->from('engine4_core_pages', 'page_id')
            ->where('name = ?', 'tfcampaign_index_manage')
            ->limit(1)
            ->query()
            ->fetchColumn();
        ...
        if(!$page_id) {
            $db->insert('engine4_core_pages', array(
                'name' => 'tfcampaign_index_manage',
                'displayname' => 'Campaign Manage Page',
                'title' => 'Campaign Manage Page',
                'description' => 'Campaign Manage Page',
                'custom' => 0
            ));
            $page_id = $db->lastInsertId();
